...
vi.mock("@/lib/gemini/client", async (importOriginal) => {
  ...
  return { ...actual, generateMealPlan: vi.fn() };
});
...
const mockGenerate = vi.mocked(generateMealPlan);
...
describe("POST /api/generate-plan", () => {
  it("returns a normalized plan on the happy path", async () => {
    mockGenerate.mockResolvedValue({ plan: makeRawPlan(), model: "gemini-2.0-flash" });
    ...
    expect(json.plan.meals[0].id).toBe("meal-breakfast");
    expect(json.plan.budget.estimatedTotal).toBe(260);
