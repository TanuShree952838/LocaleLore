...
export const rawTravelPlanSchema = z.object({
  ...
  attractions: z.array(rawAttractionSchema).min(2).max(10),
  walkingRoute: rawWalkingRouteSchema,
  localEtiquette: z.array(rawEtiquetteItemSchema).min(1).max(10),
  foodRecommendations: z.array(rawFoodRecommendationSchema).min(1).max(10),
  culturalEvents: z.array(rawCulturalEventSchema).min(1).max(10),
  artisanSpotlight: z.array(rawArtisanSpotlightSchema).min(1).max(10),
  timeline: z.array(rawTimelineSlotSchema).min(3).max(30),
  safetyTips: z.array(shortText).min(1).max(10),
  packingItems: z.array(shortText).min(1).max(20),
  localPhrases: z.array(rawLanguagePhraseSchema).min(1).max(15),
  sustainableRecommendations: z.array(shortText).min(1).max(10),
  localMythsAndLegends: z.array(rawLocalMythSchema).min(1).max(5),
